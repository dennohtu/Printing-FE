...
const Birth = ({ SegmentId }) => {
  ...
  const cols = [
    ...
    },
    {
      ...
      minWidth: 150,
      renderCell: ({ row }) => {
        if (row.motherName) return row.motherName.Animal_Tag_Number;
        else return "No Mother";
      },
    },
    {
      field: "fathername",
      ...
      minWidth: 150,
      renderCell: ({ row }) => {
        if (row.fathername) return row.fathername.Animal_Tag_Number;
        else return "No Father";
      },
    },
    {
      field: "vetName",
  ...
  const mutation = useMutation(() => {
    return axios
      .get(
        `
        ${Environment.BaseURL}/api/BirthRecords/readAllBirthRecords?Segments_ID=${SegmentId}`,
        {
          headers: {
