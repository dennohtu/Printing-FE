...
const CropSales = ({ SegmentId }) => {
  ...
  const cols = [
    ...
    },
    {
      ...
      minWidth: 150,
      renderCell: ({ row }) => {
        if (row.sellingPrice) return `Kshs ${row.sellingPrice}`;
        else return "";
      },
    },
    {
    ...
    },
    {
      ...
      minWidth: 150,
      renderCell: ({ row }) => {
        return `${row.soldBy?.User_First_Name} ${row.soldBy?.User_Last_Name}`;
      },
    },
    {
      field: "salesArea",
  ...
  const mutation = useMutation(() => {
    return axios
      .get(
        `
        ${Environment.BaseURL}/api/CropSales/readAllCropSales?Segments_ID=${SegmentId}`,
        {
          headers: {
