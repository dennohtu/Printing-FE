...
const MeatProduction = ({ SegmentId }) => {
  ...
  const cols = [
    ...
    { field: "qty", headerName: "Quantity", minWidth: 150 },
    {
      field: "sold_to",
      headerName: "Sold To",
      minWidth: 150,
    ...
    {
      field: "slaghter_date",
      headerName: "Slaughter Date",
      minWidth: 150,
      renderCell: ({ row }) => {
        if (row.slaughter_date)
          return new Date(row.slaughter_date).toDateString();
        else return " ";
      },
    },
    {
  ...
  const mutation = useMutation(() => {
    return axios
      .get(
        `
        ${Environment.BaseURL}/api/meatproduction/readAllMeatProduction?Segments_ID=${SegmentId}`,
        {
          headers: {
